...
Literal I18n 是一个面向 React / Next.js 的字面量国际化工具。你直接在组件里写原文，插件负责 AST 扫描、生成稳定 key、补齐语言 JSON，并在运行时按导航范围加载需要的翻译。
...
| 运行时数据 | 容易按语言全量加载 | 可通过 middleware/proxy + manifest 按导航范围裁剪 |
...
GIF 展示的是开发态从源码字面量到翻译文件生成的基本流程。真实项目中，建议配合 `literal-i18n gui` 管理翻译结果，配合 runtime manifest 控制导航范围 payload。
...
默认情况下，`getI18nProviderProps(locale)` 会返回当前 locale 导航树需要的 messages，而不是只返回当前 URL 的单页 messages。这是为了适配 Next.js App Router 的持久 layout：如果 `I18nProvider` 放在 `app/[locale]/layout.tsx`，从 `/zh` 客户端跳转到 `/zh/create` 时 layout 不会卸载，provider 必须已经持有 create 页需要的 key，否则页面会回退成原文。
...
如果你确认 provider 会随每个页面重新创建，或者你在页面级边界手动注入 provider，可以使用严格单页裁剪：
...
- `payloadScope`：仅 `getI18nProviderProps` 使用，默认 `navigation`；可设为 `route` 启用严格当前路由裁剪。
...
- 修复 Next App Router 持久 locale layout 下，从 `/zh` 客户端跳转到 `/zh/create` 后页面内容回退原文的问题。
- `getI18nProviderProps(locale)` 默认改为 navigation-safe payload scope，适配 layout 不卸载的客户端导航。
- 新增 `payloadScope: 'route'`，给需要严格当前路由裁剪且能保证 provider 随路由刷新的高级用法。
- `0.2.2` 的 prop `tr(...)` 抽取、alias manifest 和自动测试覆盖继续保留。
