...
| 运行时数据 | 容易按语言全量加载 | 可通过 middleware/proxy + manifest 按页面裁剪 |
...
GIF 展示的是开发态从源码字面量到翻译文件生成的基本流程。真实项目中，建议配合 `literal-i18n gui` 管理翻译结果，配合 runtime manifest 控制页面级 payload。
...
当以下条件同时满足时，`getI18nProviderProps(locale)` 会只返回当前页面需要的 messages：
