...
在 App Router 中，推荐在 locale layout 里使用 `getI18nProviderProps(locale)`。它会读取 `literal-i18n.config.*`，并自动加载当前语言 JSON、`source-map.json`、`keyMode`、`idPrefix`、`idLength`：
...
在服务端 helper 中，`source-map.json` 也会作为运行时查找辅助文件使用。比如 `getLocaleTranslator(locale)` 会先用原文查 key，查不到时再通过 source-map 找到 hash key，然后读取当前语言 JSON 的译文。
...
- `sourceMap`：手动传入 source-map；不传时默认读取 `${localeDir}/source-map.json`
- `keyMode` / `idPrefix` / `idLength`：可选。hash 模式下通常不需要传，server helper 会通过 source-map 自动查找。
