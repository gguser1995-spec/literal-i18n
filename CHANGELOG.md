...
- Demo projects now depend on the local package via `file:..`, so local verification uses the current workspace build instead of a published npm version. The Next.js 16 demo uses the `proxy.ts` convention for the pathname forwarding helper.
